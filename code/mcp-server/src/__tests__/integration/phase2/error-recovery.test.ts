  ...
  it('should handle corrupted frontmatter gracefully', async () => {
    const result = await harness.executeScenario({
      ...
      execute: async (context) => {
        ...
        await context.fs.writeFile('resources/corrupted-doc.md', `---
...
This document has corrupted frontmatter but valid content with [[resources/valid-doc]].`);
        ...
        await context.fs.writeFile('resources/partially-corrupted.md', `---
  ...
  it('should recover from missing linked documents', async () => {
    const result = await harness.executeScenario({
        ...
        return [
          {
            ...
            title: 'Main Project',
            content: 'Links to [[resources/existing]], [[resources/missing]], and [[resources/to-be-deleted]].',
            metadata: {
              title: 'Main Project',
              tags: ['project'],
              created: new Date(),
      ...
      verify: async (context) => {
        ...
        const existingBacklinksResult = context.backlinkManager.getBacklinks('resources/existing.md');
        const existingBacklinks = existingBacklinksResult.backlinks;
        IntegrationAssertions.assertBacklinks(
          'resources/existing.md',
          existingBacklinks,
  ...
  it('should handle circular link references without stack overflow', async () => {
    const result = await harness.executeScenario({
      ...
      execute: async (context) => {
        ...
        while (queue.length > 0 && visited.size < 20) { // Limit iterations
...
Links to [[resources/normal-doc]].`;
      ...
      verify: async (context) => {
        ...
        const normalBacklinksResult = context.backlinkManager.getBacklinks('resources/normal-doc.md');
        const normalBacklinks = normalBacklinksResult.backlinks;
        IntegrationAssertions.assertBacklinks(
          'resources/normal-doc.md',
          normalBacklinks,
    ...
    const result = await harness.executeScenario({
      ...
      execute: async (context) => {
        ...
        const updates = [
          ...
          context.documentUpdater.updateDocument('projects/non-existent.md', {
          ...
          context.documentUpdater.updateDocument('projects/update-test.md', {
            content: 'Updated content with [[resources/linked-resource]] and [[resources/new-link]].'
          })
        ];
      ...
      verify: async (context) => {
        ...
        const forwardLinks = await context.backlinkManager.getForwardLinks('projects/update-test.md');
