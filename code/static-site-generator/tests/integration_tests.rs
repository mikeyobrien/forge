...
fn run_para_ssg(input_dir: &Path, output_dir: &Path, verbose: bool) -> Result<(), Box<dyn std::error::Error>> {
...
fn test_empty_directory() {
    ...
    assert!(!output_dir.exists(), "Output directory should not be created for empty input");
}
...
fn test_basic_para_structure() {
    ...
    assert!(output_dir.join("projects").exists(), "projects directory not found");
    ...
    assert!(output_dir.join("areas/health/exercise-routine.html").exists());
...
fn test_navigation_and_breadcrumbs() {
    ...
    let nested_doc = fs::read_to_string(output_dir.join("projects/subproject/nested-doc.html")).unwrap();
...
fn test_edge_cases() {
    ...
    let long_doc = fs::read_to_string(output_dir.join("resources/very-long-document.html")).unwrap();
...
fn test_performance_with_many_documents() {
    ...
    assert!(duration.as_secs() < 30, "Build took too long: {:?}", duration);
